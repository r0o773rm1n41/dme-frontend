  ...
  useEffect(() => {
    ...
    const checkEligibility = async () => {
      try {
        ...
        // If payments have been closed we shouldn't allow creating an order
        const quizRes2 = await API.get('/quiz/status');
        if (quizRes2.data.state === 'PAYMENT_CLOSED') {
          alert('Payments are closed for today\'s quiz.');
          navigate('/');
          return;
        }
      } catch (error) {
