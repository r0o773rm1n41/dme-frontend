...
export default function UserBlogsPage() {
  ...
  // Live likes state
  const [totalLikesLive, setTotalLikesLive] = useState(0);
  const [statsStarAnim, setStatsStarAnim] = useState(false);

  const totalQuizzes = quizzes.length || 0;

  const joinedDate = blogUser?.createdAt
    ? dayjs(blogUser.createdAt).format("MMM D, YYYY")
    : "Recently";
  ...
  const loadUserBlogs = useCallback(async () => {
    ...
    try {
      ...
      const mappedBlogs = fetchedBlogs.map((b) => ({
        ...b,
  ...
  // Update totalLikesLive whenever blogs change
  useEffect(() => {
    const likes = blogs.reduce((sum, blog) => sum + (blog.likesCount || 0), 0);
    setTotalLikesLive(likes);
  }, [blogs]);

  // Highlight blog if coming from notification
  useEffect(() => {
  ...
  // Live like toggle with animation
  const toggleLike = async (id) => {
    try {
      ...
      setBlogs((prev) =>
        prev.map((b) =>
          b._id === id
            ? { ...b, liked: data.liked, likesCount: data.likesCount }
            : b
        )
      );
      // Animate stats star
      setStatsStarAnim(true);
      setTimeout(() => setStatsStarAnim(false), 600);
      // Update totalLikesLive instantly
      setTotalLikesLive((prev) =>
          ...
          >
            ...
            >
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "15px" }}>
                ...
                <div>
                  <h3 style={{ margin: 0, fontSize: "19px", color: "#fff" }}>
                ...
                <div style={{ textAlign: "center" }}>
                  <div
                    style={{
                      ...
                      transform: statsStarAnim ? "scale(1.25) rotate(-10deg)" : "none",
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "center",
                    }}
                  >
                    <span
                      style={{
                        ...
                        fontSize: "15px",
                        transition: "transform 0.3s",
                        transform: statsStarAnim ? "scale(1.5) rotate(-20deg)" : "none",
                        filter: statsStarAnim ? "drop-shadow(0 0 6px gold)" : "none",
                      }}
                      aria-label="stats-star"
                    ></span>
                    {totalLikesLive}+
                  </div>
                  <div
  ...
  return (
    <div className="post-card" style={{ marginBottom: 16 }}>
      ...
      <div className={`post-content ${expanded ? "expanded" : isLong ? "collapsed" : ""}`}>
        <p className="note-preview" aria-label="note-preview">{expanded ? post.content : shortContent}</p>
      ...
      <div style={{ marginTop: "8px", display: "flex", alignItems: "center" }}>
        ...
        <span className="star-count" style={{ marginLeft: "4px", fontSize: "18px", transition: "transform 0.3s", transform: starAnim ? "scale(1.2)" : "none", color: starAnim ? "gold" : undefined }}>{post.likesCount}</span>
        <span style={{ marginLeft: "15px", color: "#666" }}>👁{post.viewsCount || 0}</span>
      </div>
    </div>
  );
}
...
// const totalQuizzes =
//   blogUser?.quizHistory?.length ||
//   blogUser?.stats?.quizzes ||
//   0;
